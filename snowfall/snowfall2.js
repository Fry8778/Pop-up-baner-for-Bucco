(function ($) {
    $.fn.snowfall = function (options) {
      ...
      function createFlake(container) {
        const isFastFlake = Math.random() < 0.2; // 20% шанс сделать снежинку быстрой
        const x = Math.random() * ($(window).width() - settings.maxSize);
        ...
        const amplitude = isFastFlake ? 0 : Math.random() * 30 + 10; // Без качания для быстрых
      ...
      function animateFlake(flake, size, speed, amplitude, isFastFlake, step = 0) {
        ...
        const angleSpeed = isFastFlake ? 0 : Math.random() * 0.05 + 0.01;
  
        const animate = () => {
          top += speed;
          const oscillationX = isFastFlake ? 0 : Math.sin(step) * amplitude;
          const oscillationY = isFastFlake ? 0 : Math.cos(step * 0.5) * 2;
