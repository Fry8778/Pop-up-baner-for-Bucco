...
//     staticLabel.style.cssText = `font-size:14px;margin-bottom:5px;transition:opacity 0.5s;`;

//     const dynamicText = document.createElement("div");
//     dynamicText.className = "free-shipping-text";
//     dynamicText.style.cssText = `margin-bottom:5px;transition:all 0.5s;font-weight:bold;`;
...
//         @keyframes pulseBar { 0%,100%{transform:scaleX(1);} 50%{transform:scaleX(1.03);} }
//         .bar-pulse { animation:pulseBar 1.2s ease-in-out 1; }
//         .free-shipping-big { display:inline-flex;align-items:center;justify-content:center;width:100%;font-size:18px !important;font-weight:700 !important;text-align:center !important;margin-top:5px !important; }
//         .free-shipping-text { display:flex;align-items:center;gap:10px; }
//         .free-shipping-text img { max-width:35px;height:auto; }
//         @media(max-width:768px){
//           .free-shipping-text img{max-width:30px;}
...
//     // Якщо контейнер прогресбару зник з DOM → створюємо заново
//     if (!progressContainer || !document.contains(progressContainer)) {
//       insertProgressBar();
//     }

//     // Якщо сума не змінилась — нічого не робимо
...
//     const staticLabel = progressContainer.querySelector(
//       ".free-shipping-static",
//     );

//     // Завжди оновлюємо ширину бара
...
//     } else {
//       // Сума < 1500 → показуємо прогрес до безкоштовної доставки
...
//     // якщо бара вже немає в DOM — створюємо заново
//     if (!progressContainer || !document.contains(progressContainer)) {
//       progressContainer = createProgressBar();
//     }

//     if (target.nextElementSibling === progressContainer) return true;
...
//     // повторне оновлення після повного ререндеру
//     setTimeout(updateProgress, 200);

//     return true;
//   }

//   // === ПРОГРЕСБАР ДЛЯ МОДАЛКИ КОШИКА ===
//   let cartProgressContainer = null;

//   function insertProgressBarToCart() {
//     const popup = document.querySelector(".popup-block");
//     const cartSummary = document.querySelector(".cart-summary");

//     if (!popup || !cartSummary) return;

//     // якщо вже вставлений — нічого не робимо
//     if (
//       cartSummary.nextElementSibling &&
//       cartSummary.nextElementSibling.classList.contains(
...
//     insertProgressBar(); // checkout
//     insertProgressBarToCart(); // модалка
...
//       // якщо бар вже є — нічого не робимо
//       if (popup.querySelector(".free-shipping-progress")) return;

//       // створюємо новий бар (ВАЖЛИВО: окремий, не той що в checkout)
//       const cartProgress = createProgressBar();

//       // вставляємо перед кнопками
//       const buttons = popup.querySelector(".cart-buttons");
//       if (buttons) {
//         buttons.parentNode.insertBefore(cartProgress, buttons);
//       } else {
//         cartSummary.appendChild(cartProgress);
//       }

//       // оновлюємо і підписуємось на зміни суми
//       progressContainer = cartProgress;
//       updateProgress();
//       observeTotal();
//     }, 500);
//   }
...
(function () {
  ...
  function createProgressBar() {
    ...
    container.className = "free-shipping-progress";
    container.innerHTML = `
      <div class="free-shipping-static">🚚 Безкоштовна доставка від 1500 грн</div>
    ...
    if (!document.getElementById("freeShippingStyles")) {
      ...
      style.id = "freeShippingStyles";
      style.innerHTML = `
        .free-shipping-progress{margin:10px 0;padding:10px 15px;background:#ffe7c2;border-radius:8px}
        .free-shipping-bar-wrap{background:#e0d1c4;height:8px;border-radius:10px;overflow:hidden}
        .free-shipping-bar{background:#7b4f28;height:8px;width:0%;transition:width .3s}
        .free-shipping-text{font-weight:bold;margin-bottom:5px}
        .free-shipping-big{font-weight:700;text-align:center}
        .bar-pulse{animation:pulse 1s}
        @keyframes pulse{0%{transform:scaleX(1)}50%{transform:scaleX(1.03)}100%{transform:scaleX(1)}}
      `;
  ...
  function getTotal() {
    const node = document.querySelector(".j-total-sum");
    if (!node) return 0;
  ...
  function updateProgress(container) {
    ...
    const text = container.querySelector(".free-shipping-text");
    const label = container.querySelector(".free-shipping-static");

    if (!bar || !text || !label) return;

    bar.style.width = percent + "%";

    if (total >= freeShippingLimit) {
      text.innerHTML = `Ви отримали безкоштовну доставку`;
      label.style.opacity = "0";
      text.classList.add("free-shipping-big");

      bar.classList.add("bar-pulse");
      setTimeout(() => bar.classList.remove("bar-pulse"), 1000);
    } else {
      text.textContent = `До безкоштовної доставки залишилось ${Math.ceil(
        freeShippingLimit - total,
      )} грн`;
      label.style.opacity = "1";
      text.classList.remove("free-shipping-big");
    }
  }
  ...
  function observeTotal() {
    ...
    sumObserver = new MutationObserver(() => {
      updateProgress(checkoutContainer);
      updateProgress(cartContainer);
    });

    sumObserver.observe(node, { childList: true, subtree: true });
  }
  ...
  function insertCheckoutBar() {
    ...
    if (!checkoutContainer || !document.contains(checkoutContainer)) {
      checkoutContainer = createProgressBar();
    }

    if (
      !target.nextElementSibling?.classList.contains("free-shipping-progress")
    ) {
      target.insertAdjacentElement("afterend", checkoutContainer);
    }
  ...
  // === MODAL ===
  function insertCartBar(popup) {
    const summary = popup.querySelector(".cart-summary");
    if (!summary) return;

    if (!cartContainer || !document.contains(cartContainer)) {
      cartContainer = createProgressBar();
    }
    ...
    if (buttons) {
      buttons.parentNode.insertBefore(cartContainer, buttons);
    } else {
      summary.appendChild(cartContainer);
    }
