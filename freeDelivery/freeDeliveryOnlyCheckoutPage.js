(function () {
  // запуск тільки на checkout
  if (!window.location.pathname.includes("/checkout")) return;
  // не запускати на success page
  ...
  function observeCheckoutChanges() {
    const checkoutContainer = document.querySelector("section.checkout");
    ...
    const observer = new MutationObserver(() => {
      ...
      timeout = setTimeout(() => {
        const existingBar = document.querySelector(".free-shipping-progress");

        // якщо бар зник після перерендеру → вставляємо заново
        if (!existingBar) {
          progressContainer = null;
          insertProgressBar();
        }

        // 🔑 ПЕРЕПІДКЛЮЧАЄМО observer на суму
        observeTotal();
