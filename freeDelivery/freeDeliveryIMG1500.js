(function () {
  ...
  function updateProgress() {
    ...
    const totalText =
      document.querySelector(".j-total-sum")?.textContent || "0";
    const total = parseFloat(totalText.replace(/[^\d\.]/g, "")) || 0;
    // Видалення прогресбару після оформлення замовлення
    if (document.querySelector("section.checkout.__success")) {
      if (progressContainer && progressContainer.parentElement) {
        progressContainer.remove();
      }
      return; // не оновлюємо прогресбар
    }
