(function () {
  ...
  // 🔥 СТИЛІ ЯК У CHECKOUT (додаються 1 раз)
  if (!document.getElementById("freeShippingStyles")) {
    ...
    styleTag.textContent = `
      ...
      .bar-pulse {
        animation:pulseBar 1.2s ease-in-out 1;
      }
      ...
      .free-shipping-text { 
        ...
        align-items:center; 
        gap:10px; 
        justify-content:flex-start; /* 🔑 завжди зліва */
      }
  ...
  function createProgressBar() {
    ...
    wrapper.className = "free-shipping-wrapper";
    wrapper.appendChild(container);

    return wrapper;
  }

  function updateProgress(totalNode) {
    if (!totalNode || !cartProgressContainer) return;
    ...
    if (total >= freeShippingLimit) {
      ...
      cartProgressContainer.style.opacity = "0.95";
      ...
      cartProgressContainer.style.opacity = "1";
      dynamicText.classList.remove("free-shipping-big");
    }
  }

  function observeTotal(totalNode) {
    cartObserver?.disconnect();
    cartObserver = new MutationObserver(() => updateProgress(totalNode));
    cartObserver.observe(totalNode, { childList: true, subtree: true });
  }

  function insertProgressBar(totalNode) {
    if (!totalNode) return;
    ...
    if (totalNode.parentNode.contains(cartProgressContainer)) return;

    totalNode.insertAdjacentElement("afterend", cartProgressContainer);

    updateProgress(totalNode);
    observeTotal(totalNode);

    setTimeout(() => updateProgress(totalNode), 150);
  }

  const observer = new MutationObserver(() => {
    const totalNode = document.querySelector(".j-total-sum");
    if (!totalNode) return;
    insertProgressBar(totalNode);
  });
