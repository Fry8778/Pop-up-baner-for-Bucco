  ...
  $.fn.snowfall = function (options) {
    ...
    if (windowWidth > 768) {
      for (let i = 0; i < settings.flakeCount; i++) {
        const isFastFlake = Math.random() < 0.25; // 25% шанс сделать снежинку быстрой
    ...
    function animateFlake(flake, size, speed, amplitude, isFastFlake, step = 0) {
      ...
      const animate = () => {
        top += speed;
        const oscillationX = Math.sin(step) * amplitude; // Горизонтальное покачивание
        const oscillationY = Math.cos(step * 0.5) * 2;   // Легкое вертикальное колебание
