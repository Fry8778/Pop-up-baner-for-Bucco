(function ($) {
  $.fn.snowfall = function (options) {
    ...
    const windowHeight = $(document).height(); // Исправлено на высоту документа
    ...
    function animateFlake(flake, size, speed, step = 0) {
      ...
      const animate = () => {
        ...
        step += 0.1;    
        // Проверка выхода за границу документа
        if (top > $(document).height()) {
          top = -size;
          flake.css("top", `${top}px`);
        }    
        // Сохранение идентификатора кадра для отмены
        animationId = requestAnimationFrame(animate);
      };
    
      animate(); // Запуск анимации
    
      // Очищение анимации при удалении элемента
      flake.on("remove", () => {
