(function (t) {
  t.snowfall = function (e, i) {
    function s(s, n, a, r) {
      ...
      // Вставляем SVG инлайн в элемент
      var p = null;
      if (i.image) {
      ...
      this.update = function () {
        ...
        // Если снежинка выходит за пределы экрана, сбрасываем
        this.x + this.size > d - c || this.x < c ? this.reset() : null;
      };

      // Функция сброса снежинки
      this.reset = function () {
        this.y = 0;
        this.x = h(c, d - c);
        this.stepSize = h(1, 10) / 100;
    ...
    function n() {
      for (r = 0; r < a.length; r++) a[r].update();
    ...
    // Логика для сбора снежинок в области
    if (i.collection !== !1) {
    ...
    // Настройки внешнего вида снежинок
    i.round &&
    ...
    // Управление устройством для ориентации
    i.deviceorientation &&
