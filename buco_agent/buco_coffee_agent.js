...
@media(max-width:900px) and (orientation:landscape){
  ...
  const FLOW = [
    {
      key: "taste",
      msg: "Привіт! Я кофейний експерт Buco ☕<br>Який смак кави вам до вподоби?",
      opts: [
        "Шоколад і горіхи",
        "Фрукти та ягоди",
        "Карамель і солодощі",
    ...
    {
      key: "equip",
      msg: "Як ви зазвичай готуєте напій?",
      opts: [
        "Кавомашина або гейзер",
        "Турка",
        "Французький прес або фільтр",
        "Просто заливаю окропом",
      ],
    },
    {
      key: "level",
      ...
      opts: [
        "Новачок — хочу просте і смачне",
        "Досвідчений — люблю досліджувати",
      ],
    },
    {
      key: "budget",
      msg: "Який бюджет вам підходить?",
  ...
  const EQUIP_MAP = {
    ...
    Турка: "турка",
    "Французький прес або фільтр": "фільтр",
    "Просто заливаю окропом": "будь-яке",
  };

  const TASTE_MAP = {
    "Шоколад і горіхи": ["шоколад", "горіхи"],
    "Фрукти та ягоди": ["фрукти", "ягоди"],
    "Карамель і солодощі": ["карамель"],
    "Квіти та трави": ["квіти", "трави", "мед"],
    "Не знаю — здивуйте!": ["будь-яке"],
  };

  const LEVEL_MAP = {
    "Новачок — хочу просте і смачне": "новачок",
    "Досвідчений — люблю досліджувати": "досвідчений",
  };
...
<button class="buco-close" id="buco-close-btn">✕</button>
  ...
  function showResult() {
    const tastes = TASTE_MAP[answers.taste] || ["будь-яке"];
    ...
    const scored = PRODUCTS.map((p) => {
      ...
      if (
        tastes[0] === "будь-яке" ||
        p.forTaste.some((t) => tastes.includes(t))
      ) {
        s += 4;
      }

      // Обладнання — будь-яке дає +1, а не +2
      if (equip === "будь-яке") {
        s += 1;
      } else if (p.forEquip.includes(equip)) {
        s += 3;
      } else if (p.forEquip.includes("будь-яке")) {
        s += 1;
      }

      // Рівень

      if (p.forDev.includes(level)) s += 2;
